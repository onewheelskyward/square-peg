...
class App < Sinatra::Base
	...
	post '/ig' do
		...
		request.each do |item|
			case item["object"]
				when "geography"
					latest = HTTParty.get("https://api.instagram.com/v1/geographies/#{item["object_id"]}/media/recent?client_id=#{settings.instagram_client_id}")
					raw = RawRequest.create(type: :pubsub, method: "geographies", payload: latest.to_json)
					puts raw.valid?
					#puts latest.inspect
				when "tag"
					latest = HTTParty.get("https://api.instagram.com/v1/tags/#{item["object_id"]}/media/recent?client_id=#{settings.instagram_client_id}")
